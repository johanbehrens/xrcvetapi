...
var clients = require('./clients'); // get the mongoose model

var nodes = require('./routes/nodes');

var http = require('http').createServer(app);
clients.initClients(http);

var port = 8080;//sprocess.env.PORT || 8080;
...
apiRoutes.get('/getracestoimport', function (req, res) {
    ...
    readDirFiles.list('./imports', function (err, filenames) {
        ...
        res.json({
            success: true, races: filenames.filter(function (file, i) {
                if (i === 0) {
                    return false; // skip
                }
                return true;
...
apiRoutes.post('/heartBeat/setIp', function (req, res) {
    ...
    function done(err) {
        ...
        socketServer.setFlash(req.body.identifier, Buffer.from('01', 'hex'), done);
        function done(err) {
            if (err) {
                console.log(err);
